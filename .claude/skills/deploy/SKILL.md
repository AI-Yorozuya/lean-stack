...
1. **本機 docker 先跑通**
   - 後端+DB：`docker compose -f infra/docker-compose.local.yml up --build` → `curl localhost:8000/api/v1/health`。
   - 要試整套：build admin 前端 → `docker compose -f infra/docker-compose.prod.yml --env-file infra/.env.prod up -d --build`。
