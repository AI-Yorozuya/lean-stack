...
- **元件總表**（統一收在這一頁）：
  ...
  | 積木 | 元件 | 責任（一句話） |
  |------|------|----------------|
  | Input 輸入框 | `ui/input` | 收文字（搜尋、表單欄位） |
  | Button 按鈕 | `ui/button` | 觸發動作（含 icon 鈕） |
  | Select 下拉 | `ui/select` | 從固定選項挑一個 |
  | Switch 開關 | `ui/switch` | 兩態直接切（＝獨立狀態） |
  | Dialog 彈窗 | `ui/dialog` | 建立／編輯／確認 |
  | Label 欄名 | `ui/label` | 表單欄位的標籤 |
  | Badge 標籤 | `ui/badge` | 狀態小標 |
  | Card 卡片 | `ui/card` | 內容分區 |
  | DataTable 資料表 | `components/DataTable.vue` | 讀清單（表頭/表身、捲軸對齊都包好） |
  | Pagination 分頁 | `components/Pagination.vue` | 換頁 |
