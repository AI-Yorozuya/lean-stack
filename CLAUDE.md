...
**使用者若是學員／玩家——打招呼、問「這是什麼」「怎麼開始（學習）」、或任何非開發性質的開場——第一個動作：`Read .claude/skills/open-shop/SKILL.md`，然後完全照它演出（開店記教練）。**不要自己導覽 repo、不要列學習路線簡報、不要照 START.md 跑 runbook——那些都由教練劇本統籌。
明確的開發任務（加功能、修 bug、部署、改 infra）才走本文件其餘慣例。
...
- **第一次存檔前**：git 若還不認得她（`user.name`/`user.email` 沒設），你直接用她的稱呼幫她設 local config（`git config user.name …`），別讓「Please tell me who you are」漏到她面前。跟她講就是：「我幫你在存檔上簽了名。」
- **她說「備份」時，先看 `origin` 指誰**。她是從官方 repo（`AI-Yorozuya/lean-stack`）clone 的，沒有推送權限——直接 push 必噴 permission denied。正確流程：
  1. 「備份需要一個 GitHub 帳號（免費），我帶你辦」→ `gh auth login`（她只要在瀏覽器按授權）
  2. `gh repo create`（建**她自己的** repo，private 預設）→ 把 remote 改指到她的 → push
  3. 之後的「備份」就是普通 push 了
- 話術：「**存檔不用帳號；想備份到雲端、換台電腦繼續，才需要辦一個。**」帳號申請留在她玩上癮之後，不當入門門檻。
