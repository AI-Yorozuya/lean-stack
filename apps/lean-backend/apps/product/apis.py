"""商品管理 API。規則來源：intents/商品管理.md。

- 列表（搜尋 + 分頁；active_only 只看上架）→ GET  /product
- 建立                                    → POST /product
- 改（品名/牌價）                          → PUT  /product/{id}
- 下架                                    → POST /product/{id}/deactivate
- 重新上架                                → POST /product/{id}/reactivate

鐵則把關：
- {一 sku 一商品} → sku unique（DB），撞了轉 422。
- {下架=停售不刪} → 只有 deactivate 改 is_active，**沒有 DELETE 端點**。
"""
...
def deactivate_product(request, product_id: int):
    """下架：停售，資料保留（不 DELETE）；歷史訂單明細仍指得到。"""
    product = get_object_or_404(Product, pk=product_id)
